...
def solve_arcs_helper(argument):
    #argument['arc_number'] = ib
    #argument['twkcoeff'] = self.action.args.twkcoeff[ib]
    #argument['xsvals'] = self.action.args.xsvals
    #argument['ifuname'] = self.action.args.ifuname
    #argument['at_wave'] = self.action.args.at_wave
    #argument['at_flux'] = self.action.args.at_flux
    #argument['logger'] = logger
    # print("")
    # logger.info("BAR %d" % ib)
    # print("")
    # Starting pascal shifted coeffs from fit_center()
    b = argument['b']
    logger = argument['logger']
    coeff = argument['twkcoeff']
    # get bar wavelengths
    bw = np.polyval(coeff, argument['xsvals'])
    # smooth spectrum according to slicer
    if 'Small' in argument['ifuname']:
        bspec = b
    else:
        if 'Large' in argument['ifuname']:
            win = boxcar(5)
        else:
            win = boxcar(3)
    ...
    # logger.info("Arc spec median = %.3f, mode = %d" %
    #              (spmed, spmode.mode[0]))
    # store values to fit
    at_wave_dat = []
    at_flux_dat = []
    ...
    nrej = 0
    # loop over lines
    for iw, aw in enumerate(argument['at_wave']):
        ...
        try:
            ...
            if count < 5 or not minow or not maxow:
                rej_wave.append(aw)
                rej_flux.append(argument['at_flux'][iw])
                nrej += 1
                if verbose:
                    logger.info("Arc window rejected for line %.3f"
                                     % aw)
                continue
            ...
            if abs(cent - peak) > 0.7:
                ...
                nrej += 1
                if verbose:
                    logger.info("Arc peak - cent offset rejected "
                                     "for line %.3f" % aw)
                continue
            # store data
            ...
            at_flux_dat.append(argument['at_flux'][iw])
            # plot, if requested
            if do_inter:
                ptitle = " Bar# %d - line %3d/%3d: xc = %.1f, " \
                         "Wave = %9.2f" % \
                         (ib, (iw + 1), len(argument['at_wave']),
                          peak, aw)
            ...
            rej_flux.append(self.action.args.at_flux[iw])
            nrej += 1
    logger.info("Fitting wavelength solution starting with %d "
    ...
    # store results
    # print("")
    logger.info("Bar %03d, Slice = %02d, RMS = %.3f, N = %d" %
                     (ib, int(ib / 5), wsig, len(arc_pix_dat)))
    logger.info("RejRsd: %d, RejFit: %d" % (len(rej_rsd_wave),
                                                 len(rej_wave)))
    # print("")
    return argument['arc_number'], wfit, wsig, arc_pix_dat
...
class SolveArcs(BasePrimitive):
    ...
    def _perform(self):
        ...
        for ib, b in enumerate(self.context.arcs):
            ...
            argument['at_wave'] = self.action.args.at_wave
            argument['at_flux'] = self.action.args.at_flux
            argument['logger'] = self.logger
            argument['b'] = b
            self.action.args.fincoeff.append(0)
            bar_sig.append(0)
            bar_nls.append(0)
            my_arguments.append(argument)

        p = Pool()
        results = p.map(solve_arcs_helper, list(my_arguments))
        p.close()
        self.logger.info("Collect results")

        for result in results:
            self.action.args.fincoeff[result[0]] = result[1]
            bar_sig[result[0]] = result[2]
            bar_nsl[result[0]] = result[3]
        #bar_sig.append(wsig)
        #bar_nls.append(len(arc_pix_dat))
